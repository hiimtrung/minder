...
// (ExtendedNodePayload defined below)

// (Visual constants and type helpers moved to graph-renderer.ts)
...
interface ExtendedNodePayload extends RepositoryGraphResultNodePayload {
  direction?: string;
  distance?: number;
...
function switchTab(tabId: string): void {
  uiSwitchTab(tabId, {
    isWide: (id) => isWideGraphTab(id),
    onSwitch: (id) => {
      // Pause all sims by default to save CPU
      repoRenderer?.toggleLayout(false);
      searchRenderer?.toggleLayout(false);
      impactRenderer?.toggleLayout(false);

      setTimeout(() => {
        if (id === "graph") {
          repoRenderer?.fitToScreen();
          repoRenderer?.toggleLayout(true);
        }
        if (id === "search") {
          searchRenderer?.fitToScreen();
          searchRenderer?.toggleLayout(true);
        }
        if (id === "impact") {
          impactRenderer?.fitToScreen();
          impactRenderer?.toggleLayout(true);
        }
      }, 100);
