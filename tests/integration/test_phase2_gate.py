...
@pytest.mark.asyncio
async def test_phase2_gate(tmp_path: Path, store: RelationalStore, config: MinderConfig) -> None:
    ...
    code_hits = await tools.minder_search_code("add implementation", repo_path=str(repo_path))
    assert code_hits
