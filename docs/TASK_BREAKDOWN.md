...
### Phase 4.4 — Context Continuity and Anti-Drift Memory

**Goal**: Upgrade memory/session tools so Gemma 4 local continuously synthesizes long-flow context and prevents primary LLM drift over limited context windows.

#### P4.4-T01: Continuity Domain Contract

- **Owner**: `BE` + `ML`
- **Requirement**: Define a typed continuity packet schema shared by memory/session/query flows: `problem_summary`, `progress_summary`, `open_questions`, `risk_flags`, `next_actions`, `source_refs`, `generated_at`.
- **Result**: A stable contract exists for prompt injection and UI observability, with backward-compatible defaults.

#### P4.4-T02: Memory Recall + Synthesis Mode

- **Owner**: `ML`
- **Requirement**: Extend `minder_memory_recall` with synthesis mode that runs Gemma 4 local on top-K recall candidates to produce issue-focused summaries and suggested next actions.
- **Result**: Memory retrieval can return both raw hits and synthesized continuity output.

#### P4.4-T03: Session Snapshot Synthesis

- **Owner**: `BE` + `ML`
- **Requirement**: Extend session save/restore path to maintain a synthesized session brief generated by Gemma 4 local from state + active skills + recent memory hits.
- **Result**: Restored sessions provide concise and actionable context, not only raw serialized state.

#### P4.4-T04: Workflow-Aware Suggestion Guardrails

- **Owner**: `BE`
- **Requirement**: Constrain synthesized suggestions using workflow state (`current step`, blockers, required artifacts) so recommended actions cannot skip mandatory gates.
- **Result**: Suggested next steps remain process-compliant and auditable.

#### P4.4-T05: Prompt Injection Integration

- **Owner**: `ML`
- **Requirement**: Inject continuity packet into `minder_query` prompt assembly with token budget rules (priority fields first, truncation policy, source preservation).
- **Result**: Primary LLM receives compact, high-signal continuity context across long sessions.

#### P4.4-T06: Observability and Quality Metrics

- **Owner**: `FE` + `BE`
- **Requirement**: Add metrics for continuity quality and drift prevention (`continuity_packet_size`, `summary_regeneration_count`, `suggestion_acceptance_rate`, `restore_relevance_feedback`).
- **Result**: Operators can monitor whether continuity synthesis improves long-flow outcomes.

#### P4.4-VERIFY: Context Continuity Gate

- **Owner**: `BE` + `ML` + `FE`
- **Requirement**: Add integration/e2e tests validating:
  1. top-K recall still works in raw mode
  2. synthesis mode produces continuity packet with required fields
  3. restored sessions include latest synthesized brief
  4. workflow guardrails block invalid suggested next steps
  5. `minder_query` receives continuity packet under token budget
  6. observability counters are emitted for synthesis and restore paths
- **Result**: Context continuity layer is production-usable and reduces large-flow context loss risk.
