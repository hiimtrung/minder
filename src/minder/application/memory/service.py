...
from minder.application.continuity import compatibility_score_for_memory
...
if TYPE_CHECKING:
    from minder.application.continuity import ContinuitySynthesizer
...
class MemoryService:
    ...
    def _get_agentic_graph(self) -> Any:
        if self._agentic_graph is None:
            ...
            # Note: AgenticMemoryGraph takes tools as self. Since we are moving business logic here,
            # we can pass self (MemoryService) or we can pass a proxy. Let's see: in MemoryService,
            # passing self is perfect as the interface for MemoryService exposes the required methods.
    ...
    def _duplicate_groups(
        ...
        for index, left in enumerate(records):
            ...
            for right in records[index + 1 :]:
                ...
                similarity = MemoryService._cosine_similarity(
