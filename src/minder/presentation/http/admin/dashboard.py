    ...
    async def dashboard_static(request):
        ...
        if not is_asset_request:
            ...
            if has_admin_users and asset_path == "setup":
                target = (
                    f"{context.config.dashboard.base_path}"
            ...
            if has_admin_users and has_admin_session and asset_path == "login":
                return RedirectResponse(url=f"{context.config.dashboard.base_path}", status_code=303)
