...
> **Last updated**: 2026-04-09 (code audit + post-P4.3 dashboard routing polish captured)
...
| `Phase 4`   | Production scale, multi-user, dashboard                   | `IN PROGRESS` | `P4.3-POST polish → P4-Wave2 observability` | Need observability and broader admin/dashboard coverage; cluster and scale-up work are intentionally deferred | `Phase 4.0`, `Phase 4.1`, `Phase 4.2`, and `Phase 4.3` are complete. Post-P4.3 dashboard routing polish is in progress (3 uncommitted files). Next active milestone is `P4-T05` observability foundation. |
...
| Deferred infrastructure | `Cluster/HA/scale-up work deferred`            | `P4-T01`, `P4-T02`, `P4-T03`, `P4-T06`, and `P4-T11` stay in backlog but are not current milestones.                      |
...
| Task                                                       | Wave       | Status        | Blocker                                                             | Notes                                                                                                                                                                                                                                                             |
| ---------------------------------------------------------- | ---------- | ------------- | ------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `P4-T01` MongoDB Production Topology                       | `deferred` | `DEFERRED`    | `Explicitly deferred until cluster/scale work resumes`              | Operational store works for the current single-node/local posture; replica-set and shard hardening are not needed yet.                                                                                                                                            |
| `P4-T02` Milvus Cluster-Ready Deployment                   | `deferred` | `DEFERRED`    | `Explicitly deferred until cluster/scale work resumes`              | Current Milvus Standalone baseline is sufficient for the active scope.                                                                                                                                                                                            |
| `P4-T03` Redis HA Cache Layer                              | `deferred` | `DEFERRED`    | `Explicitly deferred until cluster/scale work resumes`              | Redis runtime works; HA/failover tuning is not part of the current milestone.                                                                                                                                                                                     |
...
| `P4-T05` Observability Stack                               | `P4-Wave2` | `NOT STARTED` | `Current next-step delivery item`                                   | OpenTelemetry, Prometheus, structured logs, and durable audit instrumentation are the next active implementation focus.                                                                                                                                           |
| `P4-T06` Production Docker Compose                         | `deferred` | `DEFERRED`    | `Explicitly deferred until broader production rollout work resumes` | Dev compose exists; production compose and secrets handling are not current priorities.                                                                                                                                                                           |
| `P4-T07` Dashboard Backend API                             | `P4-Wave4` | `PARTIAL`     | `Broader workflow/repository/user dashboard APIs still remain`      | Added browser admin sign-in/logout, dashboard client creation flow, client detail page, rotate/revoke browser actions, onboarding snippet rendering, recent activity, browser connection-test surfaces, and a browser-only acceptance gate for client management. Workflow, repository, and user admin APIs are not yet started. |
...
| `P4.3-T02` Admin Presentation Layer Split     | `P4.3-Wave1` | `DONE` | `-`     | Moved HTTP route factory and dashboard/browser handlers to [`src/minder/presentation/http/admin/routes.py`](../src/minder/presentation/http/admin/routes.py) while preserving current dashboard/admin behavior.                                                |
...
| `P4.3-POST` Dashboard Routing Polish          | `post-P4.3`  | `IN PROGRESS` | `Uncommitted — 3 files pending` | Added `ClientConsoleShell.astro` (registry shell component), no-op `middleware.ts` (prevents `[clientId]` route rewriting), updated `clients/index.astro` to use the new shell. CORS, dynamic dev-origin handling, and API URL loading also refined. These changes are unstaged/untracked and need to be committed. |
...
| `P4.3-POST`      | Dashboard Routing Polish (commit pending) | `P4.3-POST` routing + CORS + UX refinements                   | `IN PROGRESS`   |
| `P4-Wave2`       | Observability Foundation                  | `P4-T05` — `src/minder/observability/` (empty placeholder today) | `NEXT`      |
| `P4-Wave3`       | Admin and Dashboard Expansion             | `P4-T07`, `P4-T08`, `P4-T09`                                  | `NEXT`          |
